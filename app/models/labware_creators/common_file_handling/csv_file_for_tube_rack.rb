...
module LabwareCreators
  ...
  # etc.
  #
  class CommonFileHandling::CsvFileForTubeRack < CommonFileHandling::CsvFile
    validates_nested :tube_rack_scan, if: :correctly_parsed?
    validate :check_no_duplicate_well_coordinates, if: :correctly_parsed?
    validate :check_no_duplicate_tube_barcodes, if: :correctly_parsed?

    NO_TUBE_TEXTS = ['NO READ', 'NOSCAN'].freeze
    NO_DUPLICATE_WELLS_MSG = 'contains duplicate well coordinates (%s)'
    ...
    # Checks for duplicate well coordinates in the tube rack scan.
    # If any duplicates are found, an error message is added to the errors object.
    # The error message includes the duplicated well coordinates.
    # This method is used to ensure that each well coordinate in the tube rack scan is unique.
    def check_no_duplicate_well_coordinates
      ...
      errors.add(:base, format(NO_DUPLICATE_WELLS_MSG, duplicated_well_coordinates))
    end
    ...
    # @return [String] The formatted tube barcode.
    def format_position_details(row)
      format_barcode(row.tube_barcode)
    end
