...
RSpec.describe LabwareBarcode do
  shared_examples 'a labware barcode' do |input|
    ...
    describe '#=~' do
      it 'matches equivalent barcodes' do
        equivalent_barcodes.each do |equivalent_barcode|
          expect(labware_barcode =~ equivalent_barcode).to be true
        end
      end
  ...
  context 'with a code39 Sanger Barcode (DN) format' do
    ...
    it_behaves_like 'a labware barcode', {
      ean13: '1220000123724',
      machine: 'DN123H',
      human: 'DN123H'
    }
