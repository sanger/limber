...
feature 'Plate transfer', js: true do
  ...
  scenario 'starts the robot and saves the robot barcode' do
    ...
    fill_in('Scan robot', with: '123').send_keys(:return)
    within('#robot') do
      expect(page).to have_content('123')
    end
    fill_in('Scan bed', with: '580000004838').send_keys(:return)
    fill_in('Scan plate', with: plate_barcode_1).send_keys(:return)
    within('#bed_list') do
    ...
    fill_in('Scan robot', with: robot_barcode).send_keys(:return)
    within('#robot') do
      expect(page).not_to have_content('123')
      expect(page).to have_content(robot_barcode.to_s)
    end
    fill_in('Scan bed', with: '580000014851').send_keys(:return)
    fill_in('Scan plate', with: plate_barcode_2).send_keys(:return)
    within('#bed_list') do
      expect(page).not_to have_content("Robot: #{robot_barcode}")
      expect(page).to have_content("Plate: #{plate_barcode_1}")
      expect(page).to have_content('Bed: 580000004838')
