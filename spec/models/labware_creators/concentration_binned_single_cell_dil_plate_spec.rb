...
RSpec.describe LabwareCreators::ConcentrationBinnedSingleCellDilPlate do
  ...
  let(:requests) do
    Array.new(4) do |i|
      create :library_request, state: 'started', uuid: "request-#{i}", library_type: library_type_name
    end
  end
  ...
  subject { LabwareCreators::ConcentrationBinnedSingleCellDilPlate.new(api, form_attributes) }

  context 'on new' do
    it 'can be created' do
      expect(subject).to be_a LabwareCreators::ConcentrationBinnedSingleCellDilPlate
    end
