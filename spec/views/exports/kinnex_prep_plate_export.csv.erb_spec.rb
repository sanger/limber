...
RSpec.describe 'exports/kinnex_prep_plate_export.csv.erb' do
  ...
  context 'when creating a kinnex prep plate export csv' do
    ...
    let(:source_well_a1) do
      ...
      create(:well, location: 'A1', aliquots: [source_aliquot1_s1], downstream_tubes: [dest_tube1, dest_tube7])
    end
    ...
    let(:dest_aliquot7) { create(:aliquot, sample: sample2) }
    ...
    # destination tubes
    let(:dest_tube1) { create(:tube, uuid: dest_tube1_uuid, aliquots: [dest_aliquot1], name: 'SEQ:NT1O:A1') }
    let(:dest_tube2) { create(:tube, uuid: dest_tube2_uuid, aliquots: [dest_aliquot2], name: 'SPR:NT1O:A1') }
    let(:dest_tube3) { create(:tube, uuid: dest_tube3_uuid, aliquots: [dest_aliquot3], name: 'SPR:NT1O:B1') }
    let(:dest_tube4) { create(:tube, uuid: dest_tube4_uuid, aliquots: [dest_aliquot4], name: 'SEQ:NT2P:B1') }
    let(:dest_tube5) { create(:tube, uuid: dest_tube5_uuid, aliquots: [dest_aliquot5], name: 'SPR:NT2P:C1') }
    let(:dest_tube6) { create(:tube, uuid: dest_tube6_uuid, aliquots: [dest_aliquot6], name: 'SPR:NT2P:D1') }
    let(:dest_tube7) { create(:tube, uuid: dest_tube7_uuid, aliquots: [dest_aliquot7], name: 'SPR:NT2P:E1') }
