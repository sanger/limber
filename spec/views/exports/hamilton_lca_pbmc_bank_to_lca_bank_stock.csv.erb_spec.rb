...
RSpec.describe 'exports/hamilton_lca_pbmc_bank_to_lca_bank_stock.csv.erb' do
  context 'when creating a hamilton driver file csv' do
    ...
    let(:ancestor_tubes) do
      {
        ...
        sample2_uuid => ancestor_vac_tube_2
      }
    end
    ...
    let(:source_well_a1) { create(:v2_well, location: 'A1', aliquots: [source_aliquot1], downstream_tubes: [dest_tube1]) }
    let(:source_well_b1) { create(:v2_well, location: 'B1', aliquots: [source_aliquot2], downstream_tubes: [dest_tube2]) }
