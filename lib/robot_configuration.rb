...
module RobotConfiguration
  ...
  class Register
    ...
    def bravo_robot(&block)
      simple_robot('bravo',&block)
    end

    def fast_bravo_robot(&block)
      simple_robot('bravo','passed',&block)
    end

    def simple_robot(type,target_state='started',&block)
  ...
  class Simple
    ...
    attr_reader :source_purpose, :target_purpose, :layout, :type, :target_state

    def initialize(type,target_state='started',&block)
      @layout = 'bed'
      @type = type
    ...
    def from(source_purpose,bed,state='passed')
      ...
      @source_bed_barcode = bed.barcode
      @source_bed_name = bed.name
      @source_bed_state = state
    end

    def to(target_purpose,bed,state='pending')
      @target_purpose = target_purpose
      @target_bed_barcode = bed.barcode
      @target_bed_name = bed.name
